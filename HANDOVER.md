...
4. ~~Payment integration — capture-on-success / payout-on-approval flow.~~ Done against **iyzico** (not Stripe), see §6.10: real Checkout Form initiation, webhook + callback confirmation, wizard wired end to end. Subscription billing and refund/chargeback still need iyzico's Subscription/Refund APIs wired (§6.10). **You'll add real iyzico sandbox/production keys to `apps/api/.env` yourself** (not shared with the assistant, same protocol as the earlier Stripe key) — once that's in place, the checkout-form success path and full webhook flow should be re-tested against real iyzico.
...
- **`disputed → refunded | reservice → closed` is NOT implemented** — no endpoint resolves a dispute yet. That needs Admin Panel's complaint resolution UI (spec §11.1) and real refunds (spec §7.3, now iyzico's refund API — see §6.10), both later steps. An order can get stuck in `disputed` for now — expected, not a bug.
...
| Stripe Refunds API | `POST /payment/refund`, keyed on `paymentTransactionId` (**per-basket-item, not per-order like Stripe** — matters for ADIM 8's dispute flow). |
