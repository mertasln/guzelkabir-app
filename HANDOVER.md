...
## 6.16 Deployment (apps/api → DigitalOcean droplet) — spec §13, ADIM 11, done and live-verified
...
- `apps/api/Dockerfile` — single-stage (not multi-stage) by design: Prisma's client lives in `apps/api`'s own non-hoisted `node_modules/.prisma`, and a multi-stage `COPY --from=builder` of that path couldn't be verified without real Docker (unavailable in this sandbox) — mirrors CI's already-proven `npm ci` → `npm run build --workspace=apps/api` sequence instead, at the cost of image size.
  - **Two real bugs, found only by running the built image on the real droplet:** (1) `prisma generate` needs `DATABASE_URL` defined at build time (`prisma.config.ts` calls `env("DATABASE_URL")`) — an `ARG` alone didn't work (its value isn't auto-injected into `RUN`'s process env unless referenced), fixed with `RUN DATABASE_URL="$DATABASE_URL" npm ci`. (2) Container crashed with `Cannot find module '/app/apps/api/dist/main'` — root cause was a pre-existing `apps/api/tsconfig.json` gap (no `include`/`rootDir`), which swept `prisma.config.ts`/`prisma/seed.ts` into the compile and shifted all output to `dist/src/*` instead of `dist/*`. This had *also* silently broken `package.json`'s own `start:prod` script the whole time. Fixed at the source (`tsconfig.build.json`: explicit `rootDir`, excludes the two non-app files), not by changing the Dockerfile's `CMD`.
...
- **Static export (`output: "export"`), served directly by nginx — no container, no Node process.** Verified for real: a `next build` with `output: "export"` succeeds cleanly today (no `app/api` routes, no `middleware.ts`, no server-only dynamic APIs, no `next/image` anywhere in `apps/web` — all confirmed by grep, not assumed). Trade-off, not hidden: forecloses real server features (server actions, ISR) until revisited.
- **Build runs in GitHub Actions, not on the droplet** — a deliberate improvement over §6.16's own pattern: keeps Node off the shared droplet's host OS entirely, keeps the Next compiler's CPU/memory work off a box other people's processes also run on.
